...
async function main() {
  ...
  console.log(
    '✅ Core data seeded successfully (Organisation, Branch, Roles, Users, Members).',
  );

  // Seed other modules using IDs from the core data.
  // These can be run in parallel if they don't have inter-dependencies.
  await Promise.all([
    seedFinanceData(prisma, coreData).then(() =>
      console.log('✅ Finance data seeded.'),
    ),
    seedEventData(prisma, coreData).then(() =>
      console.log('✅ Event data seeded.'),
    ),
    seedCommunicationData(prisma, coreData).then(() =>
      console.log('✅ Communication data seeded.'),
    ),
    seedContentData(prisma, coreData).then(() =>
      console.log('✅ Content data seeded.'),
    ),
    seedChildrenMinistryData(prisma, coreData).then(() =>
      console.log('✅ Children ministry data seeded.'),
    ),
    seedMinistries(
      prisma,
      coreData.members,
      ...
      coreData.branch.id,
    ).then(() => console.log('✅ Ministry data seeded.')),
    seedSacraments(
      prisma,
      coreData.members,
      coreData.organisation.id,
      coreData.branch.id,
    ).then(() => console.log('✅ Sacrament data seeded.')),
    seedPrayerRequests(
      prisma,
      coreData.members,
      coreData.organisation.id,
      coreData.branch.id,
    ).then(() => console.log('✅ Prayer request data seeded.')),
  ]);
